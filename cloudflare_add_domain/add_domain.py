...
def checkDomain(cf,domain_name):
    ...
    for domain in domains:   # Iterate through the list of domains  
        ...
        if domain['name'] == domain_name:   # Check if the domain name matches  
            ...
            continue 
    print(' Domain does not exist!')    # Print a message if it doesn't exist after looping through all domains  
    return False        
...
def addDomain(cf,domain_name):
    if checkDomain(cf,domain_name):
        ...
        # Add a new DNS record to the domain's zone file  
            cf.zones.dns_records.post(zone_id, data={'type': 'A', 'name': '@', 'content': '5.181.27.133', 'ttl': 120})  

        # Add another DNS record to the domain's zone file               
            cf.zones.dns_records.post(zone_id, data={'type': 'A', 'name': 'www', 'content': '5.181.27.133', 'ttl': 120}) 
            cf.zones.dns_records.post(zone_id, data={'type': 'A', 'name': 'mobile', 'content': '5.181.27.133', 'ttl': 120}) 
